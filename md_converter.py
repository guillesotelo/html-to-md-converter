...
class HTMLToMarkdownConverter:
    ...
    def handle_table(self, tag):
        ...
        if rows:
            headers = [header.get_text().strip() for header in rows[0].find_all(["th", "td"])]
            headers = [header.replace('\n', ' <br> ').replace('-', ' ').strip() for header in headers]
            self.markdown += "*   - " + "\n    - ".join(headers) + "\n"

            for row in rows[1:]:
                cells = [self.process_cell(cell) for cell in row.find_all(["th", "td"])]
                self.markdown += "*   - " + "\n    - ".join(cells) + "\n"
    ...
    for file_name in files:
        try:
            if file_name.endswith('.html'):
                ...
                destination_file_path = os.path.splitext(
                    destination_file_path)[0] + '.md'

                with open(destination_file_path, 'w', encoding='utf-8') as file:
                    file.write(markdown_content)

                print(
                    f"\033[92m Converted: {source_file_path} -> {destination_file_path} \033[0m")
                n+=1

        except Exception as e:
            logging.error(f"\033[91m An error occurred in {file_name}: {str(e)} \033[0m")
            n+=1
